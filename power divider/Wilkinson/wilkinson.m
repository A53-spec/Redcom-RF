...
portstart = [mesh.x(end), w_50/2+w_50, substrate_thickness];
portstop  = [-port_slot/2+2*w_70 + l1_1,   w_50/2, 0];
...
stop  = [ -port_slot/2+w_70,  l1_1+w_50/2, substrate_thickness];
CSX = AddBox( CSX, 'PEC', 999, start, stop );

start = [-port_slot/2 + w_70,  l1_1 - w_70 + w_50/2, substrate_thickness];
stop  = [ -port_slot/2 + w_70 + l1_1,  l1_1 + w_50/2, substrate_thickness];
CSX = AddBox( CSX, 'PEC', 999, start, stop );

start = [-port_slot/2 + w_70 + l1_1,  w_50/2+l1_1, substrate_thickness];
stop  = [ -port_slot/2+2*w_70 + l1_1,  w_50/2, substrate_thickness];
CSX = AddBox( CSX, 'PEC', 999, start, stop );

start = [-port_slot/2,  -w_50/2, substrate_thickness];
stop  = [ -port_slot/2 + w_70, w_50/2, substrate_thickness];
CSX = AddBox( CSX, 'PEC', 999, start, stop );
...
c1_start = [-port_slot/2 + w_70 + l1_1,w_50/2 ,substrate_thickness];
c1_stop  = [-port_slot/2+2*w_70 + l1_1,-w_50/2, substrate_thickness+2];

CSX = AddLumpedElement( CSX, 'Resistor', 0, 'R', 100);
CSX = AddBox( CSX, 'Resistor', 0, c1_start, c1_stop );
